...
def movie_selected():
    ...
    if 'recommendations' not in st.session_state or st.session_state.selected_movie != st.session_state.recommendations[0]:
...
def embed_youtube_video(video_id):
    return f'<div class="video-container"><iframe src="https://www.youtube.com/embed/{video_id}?autoplay=1" frameborder="0" allowfullscreen></iframe></div>'

st.markdown(f"<h2 class='fade-in'>Selected Movie: {selected_movie}</h2>", unsafe_allow_html=True)
...
with col3:
    ...
    if trailer_link:
        st.markdown(embed_youtube_video(trailer_link.split('=')[-1]), unsafe_allow_html=True)
...
st.markdown(f"**Recommendations for movie:** {selected_movie}", unsafe_allow_html=True)
